...
function taged($tag_string){
        ...
        foreach($check_coind as $key => $value){
            $tags .= "<a href='search?search=".$key."' class='tags'><h4>".$key."</h4></a>"; //Create tags html
        }
        return $tags; 
}

function keyworded($tag_string){
    $result = str_replace('|',',',$tag_string);
    return $result;
}
